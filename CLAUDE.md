...
| `key_`  | Public API key (row id — not the bearer secret itself; see `src/lib/server/api/key.ts`) |
...
- **Resend's Broadcast API has zero engagement-stats fields.** Checked directly against Resend's API reference: create/get/list `/broadcasts` return only `id`/`status`/`created_at`/`scheduled_at`/`sent_at`-type fields — no opens, clicks, or delivered counts anywhere. Don't go looking for a stats GET endpoint that doesn't exist. `delivered`/`bounced`/`complained` only exist as webhook events (Svix-signed, see `src/lib/server/webhook.ts` and `src/routes/api/webhooks/resend`); `opened`/`clicked` are first-party as of Session 24 — see the gotcha below.
...
- **Resend's `contact.updated` webhook's `unsubscribed` flag is account-wide, not per-Topic.** There's no granular "unsubscribed from this specific Topic" event. Fine given this project's single-Segment/single-Topic-per-publication model (§10) — `unsubscribed: true` only ever means one thing here — but don't reuse `subscriber.unsubscribedAt` as evidence of anything more specific if a future session adds multiple Topics.
- **`opened`/`clicked` are first-party, not Resend-webhook-sourced, as of Session 24.** OpenLetter builds the post email HTML itself (`src/lib/server/mail.ts`), so it injects its own 1×1 open pixel and rewrites every outbound link through its own redirect route (`src/lib/server/tracking.ts`, routes at `/api/track/open` and `/api/track/click`). This was a deliberate reversal of relying on Resend's own tracking: a real production case showed `clicked` events recording while `opened` silently stopped, traced to Resend-side config (webhook event-type subscription and/or the per-domain open-tracking toggle in Domains → domain → Configuration — both still real Resend-dashboard settings, just no longer load-bearing for this app). The webhook handler (`src/routes/api/webhooks/resend`) deliberately does NOT handle `email.opened`/`email.clicked` anymore — don't re-add those branches, it would double-count against the first-party events.
- **`/api/track/click`'s `sig` query param is an open-redirect guard, not incidental.** The route 302s to whatever `url=` it's given, so without verifying `sig` (HMAC over `postId:url`, keyed off a value derived from `BETTER_AUTH_SECRET` — see `verifyClickSignature` in `src/lib/server/tracking.ts`) it would be a textbook open redirect: anyone could hand-craft a `url=` pointing anywhere and the link would 302 through the publication's own domain. A missing/invalid signature must always fall back to redirecting to `/`, never to the supplied `url`. Don't simplify this away.
- **Resend Broadcasts support exactly four merge tags in HTML content**: `{{{contact.first_name|fallback}}}`, `{{{contact.last_name|fallback}}}`, `{{{contact.email}}}`, `{{{RESEND_UNSUBSCRIBE_URL}}}`. No custom/arbitrary contact `properties` as merge tags (checked directly against Resend's docs) — this is why `/unsubscribe` identifies the reader via `{{{contact.email}}}` in the link rather than a per-recipient signed token; Resend gives us no way to inject one. The triple-brace tag must reach Resend byte-for-byte unescaped in the HTML — don't URL-encode or HTML-escape it when building the email.
- **`/unsubscribe` deliberately does NOT use the "always claim success" anti-enumeration pattern.** It reports the real outcome of the Resend API call. Reasoning: the actual send list lives in Resend's Segment/Topic membership, not in `subscriber.unsubscribedAt` — if the API call fails, the reader keeps receiving emails regardless of what our own DB or UI says, so a fake "you're unsubscribed" would be actively misleading, not just imprecise. On failure it shows the publication's `resendFromEmail` as a manual fallback contact.
