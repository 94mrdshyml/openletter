...
- **Resend's open/click tracking is off by default, per domain, and needs a verified tracking subdomain.** Not something the app can flip on for the writer — it's a DNS-verification step in Resend's own dashboard (Domains → domain → Configuration → "Enable tracking metrics"). If `dashboard/analytics` shows 0% opens/clicks even after real sends, check this before assuming a code bug — it's the single most likely cause. Documented for the writer in `dashboard/settings`.
