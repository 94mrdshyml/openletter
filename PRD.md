...
| 7   | Writer dashboard        | Subscriber count, post list, per-post `delivered`/`opened`/`clicked`/`bounced`/`complained` counts and a publication-level unsubscribed count.<sup>†</sup> Full subscriber list (`dashboard/subscribers`): email, subscribed date, newsletters received, opens, clicks, unsubscribed date if applicable. |
...
<sup>†</sup> Resend's Broadcast API returns zero engagement fields on create/get/list (checked directly against their API reference), and its own opened/clicked webhook events depend on a per-domain tracking toggle and per-endpoint event-type subscription that can silently drift out of sync (see Session 24 in `docs/SESSION_LOG.md` — this happened in practice). `opened`/`clicked` are therefore first-party as of Session 24: OpenLetter builds the email HTML itself, so it injects its own open pixel and rewrites outbound links through its own signed redirect route (`/api/track/open`, `/api/track/click` — see `src/lib/server/tracking.ts`). `delivered`/`bounced`/`complained` can't be self-hosted — they're properties of Resend's own SMTP handoff — so those stay webhook-sourced (`/api/webhooks/resend`, signature-verified). `unsubscribed` is also webhook-sourced but publication-level, not per-post: Resend's `contact.updated` event carries no `broadcast_id`, so there's no way to attribute an unsubscribe to the send that caused it.
...
**Resolved:** Custom analytics — reversed from the original §7 exclusion in Session 24, after a real production case where a writer's `opened` events silently stopped recording (Resend-side config drift, not a code bug) while `clicked` kept working. `opened`/`clicked` are now first-party (self-hosted pixel + signed click-redirect, see feature #7's footnote), removing the dependency on Resend's tracking toggle entirely. `delivered`/`bounced`/`complained` are Resend-webhook-sourced (can't be self-hosted — they're SMTP-layer facts). `unsubscribed` is webhook-sourced and publication-level only, not per-post, since Resend gives no send-attribution for it.
